...
const entry = {}
Object.keys(utils.entry).forEach(name => {
  entry[name] = ['webpack-hot-middleware/client?noInfo=true&reload=true'].concat(utils.entry[name])
})
...
const devConfig = {
  ...
  devServer: {
    ...
    host: '192.168.0.105', // 指定使用一个 host。默认是 localhost。如果你希望服务器外部可访问 0.0.0.0
    port: 8080, // 端口
    open: true, // 是否打开浏览器
